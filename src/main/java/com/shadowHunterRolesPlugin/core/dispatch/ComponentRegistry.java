 ...
 *       <b>装配期与运行期都可调用</b>（"装配即冻"取消）；id **可重复**（阶段 10 · t67 放开 ——
 ...
 * </ul>
 * <p><b>查询语义（阶段 10 · t67 按用户新路线图第 1 条统一）</b>：类型条件 = **可赋值性**
...
public final class ComponentRegistry {
     ...
     * <p><b>阶段 10 · t67（用户新路线图第 2 条）：id 唯一性已放开</b> —— 同一个 id **可以**在容器内出现多次
     ...
     * <p>阶段 10 · t67：id 可重复 ⇒ 本口移除的是**添加顺序第一个**同 id 者（与 {@link #getById(String)} 同目标）；
     ...
     * <p>阶段 10 · t67：id 可重复 ⇒ 返回的是**添加顺序第一个**同 id 者的声明
     ...
     * <p><b>阶段 10 · t67（id 可重复）</b>：本重载的**目标** = 添加顺序第一个同 id 者
     ...
     * <p><b>★ 阶段 10 · t67 的实质修正</b>：旧实现"自己不算提供者"是**按 id 排除**的
     ...
     * <p><b>阶段 10 · t67（id 可重复）的实质修正</b>：旧实现把"自己不算提供者"写成
     ...
     * <p><b>类型形参不设上界</b>（阶段 10 · t67）：旧签名是 {@code <T extends RoleComponent>}，那样
     ...
     * <p><b>不是"具体类优先"</b>（阶段 10 · t67 修正旧措辞）：旧 javadoc 写着"具体类优先"，而实现一直是
    ...
    /**
     * **按类型取全部**（阶段 10 · t67 · 用户新路线图第 1 条新增）：返回**全部**满足
     ...
     * <p>阶段 10 · t67：id 可重复 ⇒ 本读口 = "第一个"（与 {@link #get(Class)} 的"第一个"同口径、
