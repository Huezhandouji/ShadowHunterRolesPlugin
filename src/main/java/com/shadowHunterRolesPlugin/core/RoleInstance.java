 ...
 */
 ...
 //`getMaxHealth`）**已删除** —— 消费者 0（生命状态 = Bukkit 玩家属性，持有者是 `VitalsComponent`
 ...
 //`getMaxSanTE` 五个转发访问器**已删除** （消费者 0；真值与行为都在 `SanTEComponent`）。
 ...
 * 开第二条调用路径（违反"唯一受保护入口"）⇒ 调用方按 {@code instance.getAllByType(Participant.class)} **扇出**后逐个交给本方法。
