...
### §7.1 补充实证（t6 评审独立给出，队长采纳）

**① 归因形态必须钉死，且强制正/负对照。** 实测：形态 `^\s*at com\.shadowHunterRolesPlugin` 在 t2 / t4 / **t5（已知正样本）**上**都 = 0** —— 即**假绿**：连已知含插件栈帧的那一轮都抓不到。

| 形态 | t2 / t4 / t5 命中 |
|---|---|
| `^\s*at com\.shadowHunterRolesPlugin` | **0 / 0 / 0**（假绿，**不可用**） |
| `jar//com\.shadowHunterRolesPlugin` | **0 / 0 / 4** ✓ |
| `generated an exception` 或 `Global task for ShadowHunterRolesPlugin` | **0 / 0 / 1** ✓ |

⇒ **任何「插件归因异常 = 0」的结论，必须先在该批的已知正样本上验证所用形态能命中**；否则结论**不成立**（与仪器族 5「空 vs 空 = 相等」同源）。

**② 「开机窗」与「运行窗」必须分列。** 实测：t2 轮（12:56:57–12:57:13）与 t4 轮（13:08:50–13:09:05）都是**纯开机窗**（`issued server command` = 0）；唯一含 gameplay 的是 t5 轮。

⇒ 「两轮 `C = 0`」**不能**读成「运行期无归因异常」。后续批次**必须分别给出开机窗与运行窗的读数**，并写明该轮属于哪一类。
