...
public class MeiqiheziBloodySlashSkill extends Skill {
    ...
    // ───────── 阶段 13 · t93（R-6 归零）：四个协作组件引用**缓存在 start()** ─────────
    //R-4：`awake()` 只做构造期自检 / 只读自身 ⇒ 容器查找**不得**放 `awake()`；`start()` 相容器已冻结
    //⇒ 查询合法 ✓，且"一次查、处处用"（避免每次施放 / 每 tick 重复查容器）。
    //R-6：本类**不再**经服务集的白名单端口成员取用这四个能力 ⇒ 改为**向组件本身**取用 ✓
    ...
    /**
     * **开始生效**（阶段 13 · t93 新增）：把四个协作组件**一次查好**缓存进字段 ✓。
     * <p><b>为什么在 {@code start()} 而不是 {@code awake()}</b>：硬规矩 **R-4** 禁止在 {@code awake()} 里
     * **取用其他组件** ✗（awake 只做构造期自检 / 只读自身）；`start()` 相容器已冻结 ⇒ 容器查找合法 ✓。
     * <p><b>为什么缓存</b>：这些引用在一次实例生命周期内不变（组件集合在装配后固定）⇒ 重复查容器是纯浪费；
     * 旧实现经服务集端口取用，端口本身也是**构造期就持有的引用** ⇒ 缓存与旧口径同族 ✓。
     * <p><b>等价性论据</b>：四个框架级服务组件**每实例恰好一个**且已登记进实例容器（框架无条件创建 + 登记）
     * ⇒ 按类型查容器与旧端口取到的是**同一批实例** ✓（本类不参与运行期动态增删，引用不会失效）。
     * <p><b>边界（如实申报）</b>：若服务组件因**同 id 冲突**未被登记（框架对该情形记 WARNING 并跳过登记），
     * 这里的字段会是 {@code null} ⇒ 后续使用点 NPE（旧写法不会）。该形态需要角色模板里存在与服务组件同名的
     ...
     * <p><b>阶段 13 · t93（R-6 归零）</b>：四个能力改向**组件本身**取用（引用在 {@link #start()} 缓存）；
     * 伤害改走**含 {@link DamageKind} 的新入口** —— `PHYSICAL` ⇒ {@code dealtPhysicalDamage}
     * （与旧的物理伤害原语**同一条 {@code DamageUtil} 路径** ✓），来源仍是**本实例玩家** ✓
