...
> **写作口径（重要）**
> 1. 本文每条技术断言都给出 `路径:符号/行` 作为出处，**行号是 2026-09-17 17:42（UTC+8）时点的读数**，会随代码演进漂移；复核一律以「标识符 + 内容片段」为准，且**身份一律现算**（字节 / 修改时间 / SHA256 当场计算）。
...
> 4. 本文只描述**这个插件本身**（使用者/开发者视角）；各类文档的位置见 `debug-logs/README-文档索引.md`。
...
- **没有配置文件**：本工程当前不引入 `config.yml`（设计裁决见 `debug-logs/组件系统设计-Unity风格.md` §9.1 掉线即销毁），所有数值都写在 Java 里（`registry/RoleLoader.java` 的角色定义 + 各组件构造器）。
...
- **判定口径**：不要只看退出码，要看 `BUILD SUCCESSFUL` + 最新 `.class` 的 mtime 是否刷新（工程纪律见 `debug-logs/最终重构指南.md` 附录 B 命令速查（构建判定））。
...
- 起服判据（工程口径，**必须写明模式**）：见 `debug-logs/最终重构指南.md` §6.3 构建与起服验收。摘要：
...
5. **掉线**：掉线**立即销毁角色实例**（不保留、重连后没有角色）——`listener/PlayerListener.java:39-43` 调 `roleManager.clearRole(uuid)`。设计裁决见 `debug-logs/组件系统设计-Unity风格.md` §9.1 掉线即销毁。
...
| # | 事实 | 出处 |
|---|---|---|
| 1 | **掉线角色不保留**（重连后需重新 `/role set`） | `PlayerListener.java:39-43`、`debug-logs/组件系统设计-Unity风格.md` §9.1 掉线即销毁 |
| 2 | 死亡清角色时，若玩家身上还挂着本系统发放的装备被动，`stop()` 会**无条件清空**头盔/胸甲/腿甲/靴子四个槽位（连带玩家自己的装备） | `roleComponent/meiqiHezi/passive/MeiqiheziEquipmentsPassive.java:72-81`、`roleComponent/red/RedEquipmentsPassive.java:72-81`（裁决依据：`debug-logs/最终重构指南.md` §10 裁决 3，见这两处注释） |
...
- docs 全目录索引：`debug-logs/README-文档索引.md`
