...
| **② T01 无词界原文口径** | `R-[0-9]\|O-[0-9]\|D-[0-9]\|C-[0-9]\|A-[0-9]\|B-[0-9]\|T-[0-9]\|I-[0-9]\|P2\|触点\|已作废\|旧口径\|设计定案\|现算\|裁定\|硬规矩\|说明件\|本片\|本批\|台账\|护栏\|只增\|批次\|B[0-9]+\|路线图\|用户第[0-9]+条\|整理`（无词界、大小写敏感） | `eng-review` / `eng-verify`（评审侧） | 同两包：**8 行**，逐条 = 已裁定保留项 |

**★ 声明（三点，逐条可核）**：

1. **差异是「词族 + 词界」的方法差，不是严格性差**：② 比 ① **更宽**（无词界 ⇒ `栏位` 会被 `护栏` 跨词命中、`防先例` 会被 `R-1` 跨词命中）；① 比 ② **更严**（词界 ⇒ 排掉跨词假阳性）。两者**互不包含**，故读数不同是**预期的**。
2. **两套口径的结论相同**：② 的 8 行**逐条都是已裁定保留项**（`Role:477` 跨词假阳性 · `RoleInstance:1085` 字符串字面量 · `RoleInstance:227`/`:307`/`:734`/`:740`/`:755` 的 `护栏` 机制名词 · `RoleInfo:8` 的 `ADR-0006`）⇒ **无真靶残留**；① 排保留项后同样 **= 0**。
