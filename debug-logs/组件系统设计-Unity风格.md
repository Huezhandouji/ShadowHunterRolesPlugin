...
> **定位**：`debug-logs/最终重构指南.md` 第 3 章（目标架构）的展开与替换。指南里 §3.3 的接口草案、§3.6 的生命周期契约以本文为准。
...
| `buffs` | `canCastSkill()` / `canUseMainWeapon()` / `add(type,ticks)` / `has(type)` / `remainingTicks(type)` / **`applyPotionEffect(PotionEffectType type, int durationTicks, int amplifier)`（2026-09-17 R-1 裁定新增）**
> **R-1 方法级新增的约束（2026-09-17 队长裁定，成员数仍为 10）**：`buffs.applyPotionEffect` 是**新增的方法族（当前两个重载）**：① `applyPotionEffect(PotionEffectType type, int durationTicks, int amplifier)`（默认标志，供 `Unconcern`/B⑥/B⑧ 等**用默认标志**的调用点）；② **`applyPotionEffect(PotionEffectType type, int durationTicks, int amplifier, boolean ambient, boolean particles)`（2026-09-17 追加；`MeiqiheziCircleSlashSkill:47` 实测用 `ambient=true, particles=false`，3 参签名无法表达 ⇒ 若强用会**引入未申报的可见变化**（粒子由无变有、图标明暗不同），故按"标志位逐字保真"原则补此重载）**。**两者共用同一已记账路径**，用于替代组件侧 `instance.applyPotionEffect(...)`。硬要求：**必须走 `RoleInstance.applyPotionEffect` 的同一条"已记账"路径**（阶段 1 的药水记账：效果须登记以便角色清理时回收）—— **绕过记账 = 药水泄漏 = 回归**。受影响 6 处（**行号随迁移漂移，以内容片段为准**）：`MeiqiheziCircleSlashSkill`（缓慢，`slowness`）、`MeiqiheziUnconcernSkill`（速度，`speedEffect`）、`RedBleedPassive`（抗性 `RESISTANCE`，`BLEED_RESISTANCE_DURATION_TICKS`/amp 1，两处）、`RedDeeplySorrowSkill`（恢复 `REGENERATION` 45t/amp 5、力量 `STRENGTH` 45t/amp 2）。等价性表须逐条给 **效果类型 + 时长 + 增幅 + 记账条目一致** 的证据。 | 禁用/减益闸门（`canCastSkill` = 非 STUN 且非 SILENCE；`canUseMainWeapon` = 非 STUN） | 逻辑约 7 处 + 渲染 4 处（后者归渲染器）；`DefaultSanTEZero` 负责加 STUN |
...
> 行为等价性论证表见 `debug-logs/最终重构指南.md` §3.4，本次设计不改其结论，只把"上下文"换成"信号"。
...
6. **槽位冲突 / 装备回收边界 / 反馈形式 / 阶段 5.3 节流 / 阶段 6.1** → 见 `debug-logs/最终重构指南.md` §10 的裁决记录（不在此重复，避免两套说法）。
