...
function Invoke-Mode([string]$kind, [scriptblock]$extract) {
    ...
    # Write UTF-8 **without BOM** (do NOT use -Encoding ASCII: it replaces CJK with '?',
    # which collapses different-but-same-length CJK strings into one "????" and makes
    # "text unchanged" pass FALSELY).
    [System.IO.File]::WriteAllText((Join-Path $OutDir "$kind-before-$Tag.txt"), (($b -join "`r`n") + "`r`n"), (New-Object System.Text.UTF8Encoding($false)))
    [System.IO.File]::WriteAllText((Join-Path $OutDir "$kind-after-$Tag.txt"), (($a -join "`r`n") + "`r`n"), (New-Object System.Text.UTF8Encoding($false)))
    Write-Output "written -> $kind-before-$Tag.txt / $kind-after-$Tag.txt (UTF-8 no BOM)"
}
