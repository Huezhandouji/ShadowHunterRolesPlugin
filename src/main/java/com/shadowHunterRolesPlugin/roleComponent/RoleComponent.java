...
public abstract class RoleComponent {
    ...
    // ───────── 阶段 13 · t112（裁定 B1）：冷却能力的**状态面**（原 core/hotbar/CooldownBearing 迁入本组件） ─────────
    ...
    //  `HotbarRenderComponent.HotbarItem` 声明、由 `HotbarRenderComponent.HotbarPresentable` 的 default 满足 ✓）⇒ 本接口**只管状态读数** `isCooling()` ✓（接受集不变 ✓）。

    /**
     * **「这个组件有冷却」的能力接口**（阶段 6 立；阶段 8 增补状态读数；**阶段 13 · t112 迁入本类**）。
     * <p>本接口**只承载冷却的状态读数** {@link #isCooling()} ✓ —— **声明时长**（"多久"）不在此处：
     * 它由渲染族的 `HotbarRenderComponent.HotbarItem#getCooldownTicks()` 声明、并由 `HotbarRenderComponent.HotbarPresentable` 的 default 委托到描述符 ✓
     ...
     * <p><b>【已作废】旧口径原文</b>（阶段 6 原文，逐字保留）：「这个组件有冷却」的能力接口（承载**声明值与状态**两件事）；
     * 其**顶层形态位于 `core/hotbar/` 包**（已于阶段 13 · t112 删除 ✗）；也曾被 `HotbarPresentable` 的 `extends` 收进"4-in-1 能力簇"，
     * t112 起**移出该簇**（4 → 3 ✓）以避免循环继承 ✗。
     * <p><b>阶段 13 · t115 收口</b>：那个复合能力袋**整体拆解** ✗ ⇒ 本接口与 `HotbarRenderComponent.HotbarItem` /
     * `HotbarRenderComponent.HotbarItemProviding` / `EnergyComponent.EnergyCosting` 由实现者**各自显式声明** ✓
     * （见 `roleComponent/ActiveComponent` 的 implements 子句）。
     */
    public interface CooldownBearing {

        /** 冷却是否**正在进行**（本实例的冷却未到期）：与组件侧的 `isCoolingDown()` **同义** ✓；
