...
### 8.7b ★★ 词族声明（防下游把口径差异误读为「严格性不同」）

> **为什么必须有这一节**：本阶段的「靶词扫描」出现过**两套词族口径**，结论看起来不一致（一个报 0、一个报 8 行），但**两套口径下的结论相同**（都判「无真靶残留」）。若不声明，下游会把差异误读成「一套更严格 / 一套漏检」——**两者都不成立**。
> ★ **本节取数时刻（更正）**：**`10f039f` @ 2026-09-25**（★ 此前本节的行号取自 **`10f039f`**，即 `t6`/`t7` 之前的 **1134 行版** `RoleInstance`）；**现行树 = `d3c45b4`** ⇒ **条数一致、行号已下移**，两态对照见下 ② 的清单。★ 本节读数口径的**四要件**（取数时刻 / 词族 / 词界形态 / 取数域）见 `阶段7-组件架构合规审计.md` **§10**。

| 口径 | 词族（模式原文） | 谁的判据 | 现算读数 |
|---|---|---|---|
| **① 加词界口径** | `(?<![A-Za-z0-9_])…(?![A-Za-z0-9_])` 套在宽口径词表上；`护栏` 另用 `(?<![\u4e00-\u9fff])护栏(?![\u4e00-\u9fff])` | `eng-comment`（实现侧） | `core` / `listener` 两包：**排保留项白名单后 = 0** |
| **② T01 无词界原文口径** | `R-[0-9]\|O-[0-9]\|D-[0-9]\|C-[0-9]\|A-[0-9]\|B-[0-9]\|T-[0-9]\|I-[0-9]\|P2\|触点\|已作废\|旧口径\|设计定案\|现算\|裁定\|硬规矩\|说明件\|本片\|本批\|台账\|护栏\|只增\|批次\|B[0-9]+\|路线图\|用户第[0-9]+条\|整理`（无词界、大小写敏感；★ **词族 = ASCII 10 + CJK 16 = 26 族**，**不含**附加非靶词族 `栏位`） | `eng-review` / `eng-verify`（评审侧） | `core` @**`10f039f`** = **8 行**（逐条见下 ② 清单）；@**`d3c45b4`**（现行树）= **10 行** ⇒ ★ **差额 2 行** = `Role.java:623` + `RoleInstance.java:261`（来源提交 **`0cb7136`**，即队长裁定恢复「只增不改」的两处）⇒ **同一模式在两个时刻读数不同是正常的，缺取数时刻即无法复核** |

**★ 声明（三点，逐条可核）**：

1. **差异是「词族 + 词界」的方法差，不是严格性差**：② 比 ① **更宽**（无词界 ⇒ `栏位` 会被 `护栏` 跨词命中、`防先例` 会被 `R-1` 跨词命中）；① 比 ② **更严**（词界 ⇒ 排掉跨词假阳性）。两者**互不包含**，故读数不同是**预期的**。
2. **两套口径的结论相同**：② 的 **8 行**（@`10f039f`）**逐条都是已裁定保留项** ⇒ **无真靶残留**；① 排保留项后同样 **= 0**。★ **内部不一致已修正**：本节原「标称 8 行、列举 7 项」——**漏项 = `RoleInstance:946`**（`P2` 在 Java 字符串字面量内 = 行为面）；成因 = 原清单用**简写**（`Role:477`，**不含 `.java` 后缀**）⇒ 用 `([A-Za-z0-9_]+\.java):(\d+)` 切它得 **0 命中**，必须**逐项数**才是 7。**现按 8 项逐条列出**（★ 行号 = `10f039f` 实测；括号内 = 现行树 `d3c45b4` 实测，**条数一致、行号已下移**）：

   | # | 位置 @`10f039f` | 现行树 @`d3c45b4` | 定性 / 保留理由 |
   |---|---|---|---|
   | 1 | `Role.java:477` | `Role.java:477`（未移） | `栏位` 被 `护栏` **跨词误命中的假阳性**（该行从不含 护栏） |
   | 2 | `RoleInstance.java:177` | `RoleInstance.java:177`（未移） | `护栏` **机制名词**（幂等闸门） |
   | 3 | `RoleInstance.java:257` | `RoleInstance.java:257`（未移） | `护栏` **机制名词**（装配期读口闸门） |
   | 4 | `RoleInstance.java:579` | `RoleInstance.java:579`（未移） | `护栏` **机制名词**（SanTE 重入闸门状态） |
   | 5 | `RoleInstance.java:585` | `RoleInstance.java:585`（未移） | `护栏` **机制名词**（重入闸门） |
   | 6 | `RoleInstance.java:600` | `RoleInstance.java:600`（未移） | `护栏` **机制名词**（防御性设施判定） |
   | 7 | **`RoleInstance.java:946`** | `RoleInstance.java:946`（未移） | `P2` 在 **Java 字符串字面量**内 = 运行期日志文案 = **行为面**（★ 原清单漏项） |
   | 8 | `RoleInfo.java:8` | `RoleInfo.java:8`（未移） | `R-0` 是 `ADR-0006` **内子串** = **设计术语**（非过程标记） |

   ★ **现行树多出的 2 行**（`d3c45b4` = 10 行）：`Role.java:623`（`只增不改` 保留项）· `RoleInstance.java:261`（`只增不改` 保留项）—— 来源提交 `0cb7136`。★ **本节旧记的 `RoleInstance:1085` / `:227` / `:307` / `:734` / `:740` / `:755` 是更早一轮的行号**（那批经 `t30` 等卡删行后现算为 `:946` / `:177` / `:257` / `:579` / `:585` / `:600`）⇒ **逐条对照即上表**。
3. **★ 不得改本件的结论**（「对具体服务组件类的引用 = 0」等）：口径差**不影响**本件 §2/§8 的任何数字；**建议下游引用时同时写明用的是哪一套口径**（否则数字无法复核 —— 见交接文档 §7.3「报告任何计数必须给『模式原文 + 排除规则』」）。

> **补充（第 6 处口径缺口）**：两套口径**都不覆盖**裸 token 族 `[ABPT]-?[0-9]+` 与「窗口半」类 —— 已由 `t39` 记入交接文档 **§4.7**（含「已清 / 未清清单」与「标记 / 机制名词」分类口径）。本件 §2 里出现的 `A1`/`A2`/`A3`/`P4`/`P5`/`P6` 等 token **属该族**，**不在本件结论的判据范围内**（本件结论只按「具体服务组件类名 / `.class` 字面量 / 构造点」三个维度判）。
